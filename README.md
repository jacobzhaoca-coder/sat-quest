...
- **A deep, original practice pool** (unofficial estimate from `scripts/measure_pool.mjs`): **250,000+ Math variants** from procedural generators across MC, grid-in, and visual problems, plus **~5,600 distinct Reading & Writing scenarios** — a **646-item hand-authored R&W bank** (every skill-tier cell filled) combined with procedural R&W generators covering **all 11 R&W skills**. Freshness is measured as *distinct scenarios a student actually sees*, not shuffled answer-order permutations. Every question is tagged with **skill, difficulty, and a time target**, and each runthrough serves unseen questions first so repeats stay rare.
